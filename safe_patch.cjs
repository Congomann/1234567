...
if (!content.includes('Activity')) {
    content = content.replace("import { Send, FileText, Check, FileSignature } from 'lucide-react';", "import { Send, FileText, Check, FileSignature, Activity, CheckCircle2, Clock, AlertTriangle, CreditCard, X } from 'lucide-react';");
}

// 2. Add State inside PoliciesApps
...
// We know PoliciesApps ends with:
//         </div>
//     );
// };
// BUT we need to make sure we ONLY replace the one for PoliciesApps, not the whole file.

const findBlock = `                {myApps.length === 0 && (
                    <div className="text-center py-12 bg-white rounded-3xl border border-slate-200">
                        <FileText size={48} className="mx-auto text-slate-300 mb-4" />
                        <h3 className="text-lg font-black text-slate-900 mb-1">No Applications</h3>
                        <p className="text-slate-500">You don't have any active insurance applications yet.</p>
                    </div>
                )}
            </div>
        </div>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm fade-in">
                    <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col max-h-[90vh]">
                        ...
                        <div className="p-4 bg-white border-t border-slate-100 flex justify-between items-center text-xs text-slate-400 font-medium">
                            <span className="flex items-center"><Activity size={12} className="mr-1 text-indigo-500" /> <span className="text-indigo-500">Live Sync Active</span></span>
...
content = content.replace(findBlock, findBlock.replace(`        </div>
    );
};`, modalHTML));
