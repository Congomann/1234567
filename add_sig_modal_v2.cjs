...
content = content.replace(
  ...
  "Check, XSquare } from 'lucide-react';"
);

// Define SignatureModal
...
// Inject state vars
const hookSearch = `  const [formValues, setFormValues] = useState<Record<string, any>>({});`;
const hookReplace = `  const [formValues, setFormValues] = useState<Record<string, any>>({});
  const [sigModalOpen, setSigModalOpen] = useState(false);
  const [sigTargetField, setSigTargetField] = useState<any>(null);`;
content = content.replace(hookSearch, hookReplace);

// Replace prompt with modal launch
const promptSearch = `const sig = prompt(\`Enter \${field.type === 'signature' ? 'Signature' : 'Initials'}:\`, formValues[field.id] || '');
                                if (sig !== null) setFormValues({...formValues, [field.id]: sig});`;
const promptReplace = `setSigTargetField(field);
                                setSigModalOpen(true);`;
content = content.replace(promptSearch, promptReplace);

// Render image instead of text if base64
const renderSearch = `{formValues[field.id] ? (
                                <span className="font-['Brush_Script_MT',cursive] text-lg text-blue-900">{formValues[field.id]}</span>
                              ) : (`;
const renderReplace = `{formValues[field.id] ? (
...
// Inject SignatureModal at the very end of ContractingHub return
// Find the last </div>\n    </div>\n  );\n}
...
console.log('Patched ContractingHub carefully');
