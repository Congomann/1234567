# Dispatch for Test Writer Replacement (Milestone M4: Programmatic Verification Scripts)
...
1. **Script 1**: `/Users/newholland/1234567/scripts/verify-session-tracking.mjs`:
   ...
   - Verifies that these 3 visits are successfully grouped and stored as a single unified session in the database (Firestore document store via `backend/services/behavioralTrackingService.cjs`).
...
2. **Script 2**: `/Users/newholland/1234567/scripts/verify-carrier-adapter.mjs`:
   - Executes the mock carrier adapters (`AcmeMutualAdapter` and `ApexLifeAdapter` from `services/carrier/index.ts`) with dummy API payloads.
...
- Read `/Users/newholland/1234567/.agents/explorer_bt_survey_3/handoff.md`.
...
- Execute `npm run test:all` (all pass).
- Deliver your handoff report to `/Users/newholland/1234567/.agents/test_writer_m4_2/handoff.md`.
