# Dispatch for Worker M3 Replacement: CRM Admin UI & Client Views Integration
...
2. **R2 Normalized Client Policy View** (`pages/crm/Clients.tsx` reachable at `/crm/clients`):
   ...
   - Implement `components/crm/NormalizedPolicySection.tsx` rendering all 6 normalized policy fields:
     ...
     - Carrier badge (Acme Mutual / Apex Life) and an interactive "Sync Carrier Data" button that executes carrier normalization (from `services/carrier/index.ts`) and updates the client display in real-time.
3. Design System Compliance:
   - Use existing Tailwind CSS classes, Apple glassmorphic utilities (`.apple-glass`, `.apple-card`), and Lucide React icons.
...
- Deliver your handoff report to `/Users/newholland/1234567/.agents/worker_m3_2/handoff.md`.
