    ...
    <div className="w-full lg:w-[24rem] flex-shrink-0 bg-white rounded-[2.5rem] shadow-sm border border-slate-200 flex flex-col overflow-hidden h-full">
      <div className="p-6 border-b border-slate-100 bg-slate-50 flex gap-4">
        <button 
            ...
            className={`flex-1 py-3 text-sm font-black uppercase tracking-widest rounded-xl transition-all ${activeTab === 'agenda' ? 'bg-slate-900 text-white shadow-md' : 'bg-slate-200 text-slate-500 hover:bg-slate-300'}`}
        >
            Agenda
        </button>
        <button 
            onClick={() => setActiveTab('tasks')}
            className={`flex-1 py-3 text-sm font-black uppercase tracking-widest rounded-xl transition-all ${activeTab === 'tasks' ? 'bg-slate-900 text-white shadow-md' : 'bg-slate-200 text-slate-500 hover:bg-slate-300'}`}
        >
            Tasks
        </button>
      </div>
