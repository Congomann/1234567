...
function createPage(componentName, title, date, sections) {
  let sectionsHtml = sections.map((sec, i) => {
    return `
          <section>
            ${sec.title ? \`<h2 className="text-2xl font-bold text-[#0B2240] mb-4">\${sec.title}</h2>\` : ''}
            <div className="text-slate-600 leading-relaxed space-y-4 font-medium">
              ${sec.paragraphs.map(p => {
                if (p.startsWith('- ')) {
                   // list item (not really strictly parsed here, but we can just output p)
...
// Note: I added dangerouslySetInnerHTML so the <a> tags render correctly. 
// Wait, the createPage template just emits it as raw text if I don't use dangerouslySetInnerHTML.
// Let's modify createPage to dangerously set inner html.
