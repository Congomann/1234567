            ...
            <div className="flex flex-col gap-4 p-4 bg-slate-50 rounded-xl border border-slate-200">
                <div className="flex items-center justify-between">
                    ...
                    <button 
                        ...
                        className={`relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors  ease-in-out focus:outline-none ${formData.micrositeEnabled ? 'bg-green-500' : 'bg-slate-300'}`}
                    >
                        <span className={`pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition  ease-in-out ${formData.micrositeEnabled ? 'translate-x-5' : 'translate-x-0'}`} />
                    </button>
                </div>
                ...
                {formData.micrositeEnabled && user && (
                    <div className="pt-4 border-t border-slate-200 flex items-center justify-between">
                        <div>
                            ...
                            <a 
                                href={`https://newhollandfinancial.com/advisor/${user.name.toLowerCase().replace(/ /g, '-')}`} 
                                target="_blank" 
                                rel="noopener noreferrer"
                                className="text-sm font-medium text-blue-600 hover:underline mt-1 block"
                            >
                                {`https://newhollandfinancial.com/advisor/${user.name.toLowerCase().replace(/ /g, '-')}`}
                            </a>
                        </div>
                        <button 
                            type="button"
