...
export const Onboarding: React.FC = () => {
    ...
    const handleStatusUpdate = (status: 'Approved' | 'Rejected') => {
        if (!isAdmin) {
            alert('STRICT PERMISSION: Specialized Administrator status required for onboarding.');
            return;
        }
        if (selectedApp) {
            if (status === 'Approved') {
                        ...
                        {selectedApp.status === 'Pending' && (
                            <div className="p-8 border-t border-slate-100 bg-slate-50 flex gap-4">
                                ...
                                <button
                                    ...
                                    className={`flex-1 py-4 font-bold rounded-xl shadow-lg transition-all flex items-center justify-center gap-2 ${isAdmin ? 'bg-green-600 text-white hover:bg-green-700 shadow-green-600/20' : 'bg-slate-100 text-slate-400 cursor-not-allowed'}`}
                                    title={!isAdmin ? "Administrator (Admin) check required" : ""}
                                >
                                    {!isAdmin && <Lock className="h-4 w-4" />}
                                    <UserPlus className="h-5 w-5" /> {isAdmin ? 'Approve & Onboard' : 'Admin Approval Required'}
                                </button>
                            </div>
                        )}
                    </div>
